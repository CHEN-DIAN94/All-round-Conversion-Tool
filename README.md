...
### 🛡️ 运行时监控
- Qt C++ 层崩溃信号重定向（qDebug / qWarning / qCritical / qFatal）
- QThread 异常追踪（启动/退出/未捕获异常）
- faulthandler 段错误捕获
- 带时间戳的 trace 日志，精确到毫秒
...
- **失败重试** — 一键重试所有失败文件
- **转换历史** — 记录每次转换，支持搜索和导出
- **任务派发注册表** — 集中管理任务 key 到执行器的映射，替代大型 if/elif
- **强类型任务模型** — TaskSpec / ExecutionResult 数据模型
- **CLI 模式** — 命令行批量处理，可脚本化
...
### 前置要求
- **Python 3.10+**
- **FFmpeg 6.0+** — 视频/音频转换需要，放入 `bin/` 目录或添加到 PATH
...
├── task_models.py           # TaskSpec / ExecutionResult 数据模型（Phase 1 重构）
├── execution_registry.py    # 任务派发注册表（task key → 执行器映射）
├── presets.py               # 转换预设管理
├── history.py               # 转换历史（JSON 持久化）
├── logging_config.py        # 全局日志配置（按日轮转，统一格式）
├── monitor.py               # 运行时监控（Qt 崩溃信号重定向 / faulthandler）
├── build_portable.py        # 免安装版打包脚本（Python embeddable）
├── engines/
...
│   ├── gpu_scheduler.py     # GPU 硬件加速智能调度（NVENC/QSV/AMF）
...
└── tests/
    ...
    └── test_execution_registry.py   # 任务派发注册表测试
```
...
项目使用 Python embeddable 包生成免安装版，无需 PyInstaller。

```bash
# 准备：将 ffmpeg.exe 放入项目 bin/ 目录
# 然后执行：
python build_portable.py

# 产物：dist/流光-Portable/  →  压缩成 zip 即可分发
```
...
| 模块 | 说明 |
|------|------|
| **任务派发注册表** (`execution_registry.py`) | 集中定义 task key → 执行器映射，替代 workers.py 中的大型 if/elif 链，便于扩展新功能 |
| **强类型任务模型** (`task_models.py`) | `TaskSpec` / `ExecutionResult` dataclass，替代散落的字符串和裸 dict，提升代码可维护性 |
| **GPU 智能调度** (`gpu_scheduler.py`) | 自动检测 GPU 类型（NVIDIA/Intel/AMD），管理并发会话限制，满载自动回退软编码，线程安全 |
| **运行时监控** (`monitor.py`) | 重定向 Qt 消息到日志、追踪 QThread 异常、faulthandler 段错误轮询，解决"窗口直接消失"类崩溃排查难题 |
| **统一日志框架** (`logging_config.py`) | 按日滚动日志文件，控制台 + 文件双 handler，统一 `[时间] [级别] 模块: 消息` 格式 |
| **UI Mixin 架构** | `ui.py` 组合 `ui_file_table.py` / `ui_conversion.py` / `ui_settings.py`，职责清晰，降低主窗口复杂度 |
| **免安装打包** (`build_portable.py`) | 基于 Python embeddable，生成即开即用的便携版，无需用户安装 Python 环境 |

---

<p align="center">
  <sub>流光 v1.1.0 — 格式流转，光影随行</sub>
</p>
